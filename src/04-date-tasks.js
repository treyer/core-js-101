...
function timeSpanToString(startDate, endDate) {
  ...
  const min = `0${endDate.getMinutes() - startDate.getMinutes()}`.slice(-2);
  const sec = `0${endDate.getSeconds() - startDate.getSeconds()}`.slice(-2);
  const ms = `00${
...
function angleBetweenClockHands(date) {
  ...
  const hoursAngle = (60 * hours + minutes) / 2;
  const minutesAngle = 6 * minutes;

  let angleInDeg = Math.abs(hoursAngle - minutesAngle);
